...
# Vectorized Julia set function with early exit
def julia_set(c):
    z = np.zeros((resolution, resolution), dtype=np.complex128)
    n = np.zeros((resolution, resolution), dtype=int)
    
    # Create a mask for points that haven't escaped
    mask = np.ones((resolution, resolution), dtype=bool)

    for i in range(max_iterations):
        # Update only the points that haven't escaped
        z[mask] = z[mask] ** 2 + c
        mask = mask & (np.abs(z) <= 2)  # Update mask for points that still haven't escaped
        n[mask] += 1
